...
router.get('/', (req, res) => {
  try {
    ...
    if (projectId) {
      if (projectId === 'none') {
        conditions.push('p.project_id IS NULL');
      } else {
    ...
    if (tagIds) {
      ...
      query += ` JOIN photo_tag_assignments pta ON p.id = pta.photo_id`;
      conditions.push(`pta.tag_id IN (${tags.map(() => '?').join(',')})`);
      params.push(...tags);
    }
    ...
    if (photos.length > 0) {
      const tagQuery = db.prepare(`
        SELECT pt.*, pta.photo_id
        FROM photo_tags pt
        JOIN photo_tag_assignments pta ON pt.id = pta.tag_id
        WHERE pta.photo_id IN (${photos.map(() => '?').join(',')})
      `);
      const tagRows = tagQuery.all(...photos.map(p => p.id)) as any[];
      tagRows.forEach((tag) => {
        const photo = photos.find(p => p.id === tag.photo_id);
        if (photo) {
...
router.post('/upload', upload.array('photos', 20), (req, res) => {
  try {
    ...
    const insertPhoto = db.prepare(`
      INSERT INTO photos (id, name, type, mime_type, size, path, project_id)
      VALUES (?, ?, 'local', ?, ?, ?, ?)
    `);

    const insertTagAssignment = db.prepare(`
      INSERT OR IGNORE INTO photo_tag_assignments (id, photo_id, tag_id)
      VALUES (?, ?, ?)
    `);

    const uploadedPhotos: FormattedPhoto[] = [];
...
router.put('/:id/tags', (req, res) => {
  try {
    ...
    db.transaction(() => {
      db.prepare('DELETE FROM photo_tag_assignments WHERE photo_id = ?').run(req.params.id);
      for (const tagId of tagIds) {
        insertTagAssignment.run(uuidv4(), req.params.id, tagId);
      }
    })();
      ...
      for (const photoId of photoIds) {
        if (action === 'remove') {
          db.prepare(`
            ...
            WHERE photo_id = ? AND tag_id IN (${tagIds.map(() => '?').join(',')})
          `).run(photoId, ...tagIds);
        } else if (action === 'replace') {
          db.prepare('DELETE FROM photo_tag_assignments WHERE photo_id = ?').run(photoId);
          for (const tagId of tagIds) {
...
router.post('/bulk-assign-project', (req, res) => {
  try {
    ...
    db.prepare(`
      UPDATE photos SET project_id = ? WHERE id IN (${photoIds.map(() => '?').join(',')})
